...
namespace Alura.Estacionamento.Modelos
{
    public class Patio
    {
        ...
        public Veiculo AlteraDadosVeiculo(Veiculo veiculoNovosDados)
        {
            var veiculoEncontrado = (from veiculo in this.Veiculos
                                     where veiculo.Placa == veiculoNovosDados.Placa
                                     select veiculo).SingleOrDefault();

            veiculoEncontrado.AlteraDados(veiculoNovosDados);
            return veiculoEncontrado;
        }

        private string GerarTicket(Veiculo veiculo)
        {
            veiculo.IdTicket = new Guid().ToString().Substring(0, 10);
            veiculo.Ticket = $">>> Identificador: {veiculo.IdTicket}" +
                            $">>> Data/Hora de Entrada: {DateTime.Now}" +
                            $">>> Placa do Veículo: {veiculo.Placa}" +
                            $">>> Operador Pátio: {OperadorPatio}";
